...
model_benchmark_V5 <- function(Features,
                               Target,
                               Input_Data,
  ...
  evaluate_with_optimal_threshold_cross_validation <- function(training_pred, testing_pred,
  ...
  if (Enable_prechecking & (fraction_below < cutoff_Lower || fraction_below > cutoff_Upper)) {
    ...
    for (MLmodel in ML_model) {
      if (MLmodel == "Random Forest") {
        ...
        # Calculate the SHAP for each fold during the cross-validation
        ...
        for (i in seq_along(folds)) {
          ...
          if (model_type == "Classification") {
            # Best Accuracy from results
            # Probabilities for class 1
            ECN.model.train.prob <- predict(fold_model, train_df_fold, type = "prob")[, 2]
            ECN.model.predict.prob <- predict(fold_model, test_df_fold, type = "prob")[, 2]

          } else if (model_type == "Regression") {
            # Predicted values (continuous)
            ECN.model.train.prob <- predict(fold_model, train_df_fold)
            ECN.model.predict.prob <- predict(fold_model, test_df_fold)
          }


          AUC_evaluation_results_cross_validation <- evaluate_with_optimal_threshold_cross_validation(
            training_pred = ECN.model.train.prob,
            testing_pred = ECN.model.predict.prob,
            train_labels = train_df_fold[[Dependency_gene]],
            test_labels = test_df_fold[[Dependency_gene]],
            positive_class = "1",
            model_type = model_type,
            Finding_Optimal_Threshold = Finding_Optimal_Threshold
          )

          feature_importance <- rank_feature_importance_caret(fold_model)
          ...
          final_benchmark_result <- rbind(
            ...
            data.frame(
              ...
              # Classification metrics
              Optimal_Threshold = if (model_type == "Classification") AUC_evaluation_results_cross_validation$optimal_threshold else NA,
              Training_Accuracy = if (model_type == "Classification") AUC_evaluation_results_cross_validation$training_accuracy else NA,
              Training_Precision = if (model_type == "Classification") AUC_evaluation_results_cross_validation$training_precision else NA,
              Training_Recall = if (model_type == "Classification") AUC_evaluation_results_cross_validation$training_recall else NA,
              Training_F1 = if (model_type == "Classification") AUC_evaluation_results_cross_validation$training_F1 else NA,
              Training_Kappa = if (model_type == "Classification") AUC_evaluation_results_cross_validation$training_Kappa else NA,
              Training_AccuracyPValue = if (model_type == "Classification") AUC_evaluation_results_cross_validation$training_accuracyPValue else NA,
              Training_McnemarPValue = if (model_type == "Classification") AUC_evaluation_results_cross_validation$training_McnemarPValue else NA,
              Training_AUROC = if (model_type == "Classification") AUC_evaluation_results_cross_validation$training_auroc else NA,
              Prediction_Accuracy = if (model_type == "Classification") AUC_evaluation_results_cross_validation$testing_accuracy else NA,
              Prediction_Precision = if (model_type == "Classification") AUC_evaluation_results_cross_validation$testing_precision else NA,
              Prediction_Recall = if (model_type == "Classification") AUC_evaluation_results_cross_validation$testing_recall else NA,
              Prediction_F1 = if (model_type == "Classification") AUC_evaluation_results_cross_validation$testing_F1 else NA,
              Prediction_Kappa = if (model_type == "Classification") AUC_evaluation_results_cross_validation$testing_Kappa else NA,
              Prediction_AccuracyPValue = if (model_type == "Classification") AUC_evaluation_results_cross_validation$testing_accuracyPValue else NA,
              Prediction_McnemarPValue = if (model_type == "Classification") AUC_evaluation_results_cross_validation$testing_McnemarPValue else NA,
              Prediction_AUROC = if (model_type == "Classification") AUC_evaluation_results_cross_validation$testing_auroc else NA,

              # Regression metrics
              Training_RMSE = if (model_type == "Regression") AUC_evaluation_results_cross_validation$training_rmse else NA,
              Training_MAE = if (model_type == "Regression") AUC_evaluation_results_cross_validation$training_mae else NA,
              Training_R2 = if (model_type == "Regression") AUC_evaluation_results_cross_validation$training_r2 else NA,
              Prediction_RMSE = if (model_type == "Regression") AUC_evaluation_results_cross_validation$testing_rmse else NA,
              Prediction_MAE = if (model_type == "Regression") AUC_evaluation_results_cross_validation$testing_mae else NA,
              Prediction_R2 = if (model_type == "Regression") AUC_evaluation_results_cross_validation$testing_r2 else NA,
        ...
        } # End of for (i in seq_along(folds)) loop
