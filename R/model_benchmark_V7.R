...
model_benchmark_V7 <- function(
  ...
  for (i in seq_along(outer_folds)) {
    ...
    for (method in models) {
      ...
      if (shap) {
        ...
        # background is always from training data
        bg_df <- as.data.frame(X_tr[bg_ix, , drop = FALSE])

        # Define which rows to explain for each dataset
        X_explain_train <- as.data.frame(
          if (is.finite(shap_pred_max)) {
            X_tr[seq_len(min(nrow(X_tr), shap_pred_max)), , drop = FALSE]
          } else {
            X_tr
          }
        )
        X_explain_test <- as.data.frame(
          if (is.finite(shap_pred_max)) {
        ...
        # =====================================================
        # loop over Dataset = "Train" and "Test"
        # =====================================================
        for (Dataset in c("Train", "Test")) {

          X_explain_df <- if (Dataset == "Train") X_explain_train else X_explain_test
          if (!nrow(X_explain_df)) next
          ...
          if (is_xgb || is_ranger) {
            ...
            saveRDS(
              ...
              file = file.path(
                outdir,
                sprintf("%s_%s_Fold%d_%s.treeshap.rds", target_var, method, i, Dataset)
              )
            )
            ...
            if (TS_INTERACTIONS && !is.null(ts$interactions)) {
              ...
              readr::write_csv(
                ...
                file.path(
                  outdir,
                  sprintf(
                    "%s_%s_Fold%d_%s_SHAP_interactions.csv",
                    target_var, method, i, Dataset
                  )
                )
              )
            ...
            # ---- Also save main effects as CSV (optional) ----
            ...
            readr::write_csv(
              ...
              file.path(
                outdir,
                sprintf(
                  "%s_%s_Fold%d_%s_SHAP_mainEffects.csv",
                  target_var, method, i, Dataset
                )
              )
            )
            ...
            if (save_plots) {
              pdf(file.path(
                outdir,
                sprintf("Fold%d_%s_%s_SHAP_importance.pdf", i, method, Dataset)
              ))
            ...
            saveRDS(
              ...
              file = file.path(
                outdir,
                sprintf("%s_%s_Fold%d_%s.kernelshap.rds", target_var, method, i, Dataset)
              )
            )
  ...
  if (!is.null(shap_df)) {
    ...
    shap_df_summary <- shap_df %>%
      ...
      summarise(
        MeanAbsSHAP = mean(MeanAbsSHAP, na.rm = TRUE),
        .groups = "drop"
      ) %>%
      arrange(Dataset, dplyr::desc(MeanAbsSHAP)) %>%
